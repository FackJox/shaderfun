...
void main() {
    ...
    float diff = max(dot(norm, lightDir), -01.0);
    ...
    // newFakeUv = fract(newFakeUv + vec2(uTime/10.0, uTime/10.0));
    //     float fresnel = dot(i.worldNormal, float3(0, 1, 0));
    float fresnel = abs(dot(vec3(0.0, 1.0, 0.0), norm));
    fresnel = fresnel * fresnel * fresnel * fresnel * fresnel * fresnel * fresnel;


    vec2 modifiedUv = fract(vUv + vec2(uTime/20.0, uTime/20.0));
    vec4 texture = texture2D(abstractTexture, modifiedUv + 0.1 * cnoise(vec4(modifiedUv*10.0, uTime/100.0, 0.0), vec4(5.0)));
    ...
    gl_FragColor = vec4(mix( vec4(diffuse, 1.0) * vec4( mix(vec3(1.0),texture.rgb, 1.0 - fresnel), 1.0), vec4( mix(vec3(1.0),texture.rgb, 1.0 - fresnel), 1.0), 1.0 * sin(uTime/5.0+1.0)));
